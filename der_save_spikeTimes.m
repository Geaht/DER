        ...
        for idx = 1:no_channels
            ...
            if exist(timesfile,'file') && exist(spikesfile,'file')
                ...
                if sum(index_currSp) > 0
                    ...
                    cluster_info(3,idx) = {ones(1,size(clusterIDList(clusterIDList~=0),1))};
                ...
                if exist(timesfile,'file')
                    error(sprintf('Missmatch: %s is missing',spikesfile))
                else
                    error(sprintf('Missmatch: %s is missing',timesfile))
                end
